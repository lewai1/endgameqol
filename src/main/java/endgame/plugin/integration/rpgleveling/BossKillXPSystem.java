...
/**
 * OnDeathSystem that awards RPG Leveling XP when a boss NPC dies.
 * Only registered when RPG Leveling integration is active.
 */
public class BossKillXPSystem extends DeathSystems.OnDeathSystem {

    private final EndgameQoL plugin;
    private final RPGLevelingBridge bridge;

    public BossKillXPSystem(EndgameQoL plugin, RPGLevelingBridge bridge) {
        this.plugin = plugin;
        this.bridge = bridge;
    }
    ...
    public void onComponentAdded(@Nonnull Ref<EntityStore> ref, @Nonnull DeathComponent component,
        ...
        try {
            ...
            plugin.getLogger().atWarning().log("[BossKillXP] Error awarding XP: %s", e.getMessage());
        }
    }
